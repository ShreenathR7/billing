...
class Test_Logimax:
    ...
    def test_Billing_purchase(self,booting_function):   
        ...
        sleep(5)
        self.driver.find_element(by=By.ID,value=locators.Logi_Locators().sales_and_Purchase).click()
        Sale_button = self.driver.find_element(By.XPATH, '(//*[@class="custom-label"])[2]').text
        assert self.driver.title == 'Logimax Technology | Admin'
        print('Billing to : {a}'.format( a= Sale_button))

        sleep(5)
        self.driver.find_element(by=By.ID,value=locators.Logi_Locators().Esti).send_keys(data.Logi_Data().Esti_Number)    
        assert self.driver.title == 'Logimax Technology | Admin'
        print('Estimation Number: {a}'.format(a = data.Logi_Data().Esti_Number))
        sleep(5)
